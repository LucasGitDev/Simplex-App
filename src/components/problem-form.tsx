...
export function ProblemForm({ onSubmit, initialProblem }: ProblemFormProps) {
  ...
  const [constraintNames, setConstraintNames] = useState<string[]>(["Restrição 1", "Restrição 2", "Restrição 3"])
  ...
  useEffect(() => {
    if (initialProblem) {
      ...
      setConstraintMatrix(initialProblem.constraintMatrix.map((row) => [...row]))
  ...
  const handleConstraintCoefficientChange = (row: number, col: number, value: string) => {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input id="numVars" type="number" min="1" value={numVars} onChange={handleNumVarsChange} />
        </div>
        <div className="space-y-2">
      ...
      <div className="flex items-center space-x-2">
        <Switch id="maximize" checked={maximize} onCheckedChange={setMaximize} />
        ...
        <div className="grid grid-cols-4 gap-2">
          {variableNames.map((name, i) => (
            <div key={`var-${i}`} className="space-y-1">
              ...
              <Input id={`var-${i}`} value={name} onChange={(e) => handleVariableNameChange(i, e.target.value)} />
            </div>
          ))}
        </div>
      </div>
            ...
            <div className="grid grid-cols-5 gap-2 items-end">
              {row.map((coeff, j) => (
                <div key={`constraint-${i}-${j}`} className="space-y-1">
                  <Label htmlFor={`constraint-${i}-${j}`}>{variableNames[j]}</Label>
                  <Input
                    id={`constraint-${i}-${j}`}
                    type="number"
                    step="any"
